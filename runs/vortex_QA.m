...
% resolution              = 2;
%NN(1)                   = 28*resolution+1;                               % number of nodes in X direction
%NN(2)                   = 28*resolution+1;                               % number of nodes in Y direction
NN(1)                   = 20*resolution+1;                               % number of nodes in X direction
NN(2)                   = 20*resolution+1;                               % number of nodes in Y direction
...
% start the algorithm
% video
%timestep = 80;     % number of frame to save %originally 20
		videofps=60;                              % video output fps
		timescale=dt*videofps; % 1:1 frame to timestep(dt)

timestep = videofps*ftime/timescale;
r=timestep/5/5;      % number of frame per second video ~200s
...
mkdir('OutputDump');
tt=1;
load("D:\Users\Tito\Documents\QA Vortex\QA Vortex 200105-01\OutputDump\399.mat");
for tt = tt:timestep
...
%     ft=ftime;
	 while t<ft+0.0001      
		...
		for sp = 1:spCount
			...
			%b_sp(sp,1)  = (b_r(sp) * cos(beta(sp)) - b_angle(sp) * sin(beta(sp)))/2;
			b_sp(sp,1)  = b_r(sp) * cos(beta(sp)) - b_angle(sp) * sin(beta(sp));
			%b_sp(sp,2)  = (b_r(sp) * sin(beta(sp)) + b_angle(sp) * cos(beta(sp)))/2;     
		 ...
		 switch Version
			...
			nfbcx,nfbcy,fbcx,fbcy,F_sp,V_spo,dt);
			 ...
			 case 'CPDI'
			[v_ssp,x_sp,d_sp,F_sp,V_sp,s_sp,p_sp,r1_sp,r2_sp] = CPDI_solver(CModel,...
			nodeCount,spCount,cellCount,x_sp,x_spo,d_sp,le,NN,LOC,b_sp,V_sp,ptraction_sp,v_ssp,s_sp,m_sp,p_sp,...
			nfbcx,nfbcy,fbcx,fbcy,E,nu,F_sp,V_spo,dt,r1_sp,r10_sp,r2_sp,r20_sp);
...
%         caxis([0 0.1]);
	...
	saveas(StressProfile1,['OutputDump\\' 'tt=' num2str(tt) ', t=' num2str(t) '.png']);
	1;
	...
	save(['OutputDump\\' num2str(tt) '.mat'],'-v7.3',...
		'v_ssp', ...
		'x_sp', ...
		'd_sp', ...
		'F_sp', ...
		...
		'vel_ana', ...
		'Cauchy_ana');
	%"D:\Users\Tito\Documents\QA Vortex\QA Vortex 200105-01\OutputDump\398.mat"
	1;
end
close(writerObj2);
